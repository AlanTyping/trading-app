...
export default function EliminarTodoConfirmación() {
  ...
  return (
    ...
    items-center justify-center absolute inset-x-0 bottom-[0%] top-[-30%] bg-[#061333] z-[5]'>
      <div className='h-[50%] w-[100%] flex items-center text-[1.4rem] justify-center relative'>
        <p className="absolute top-[20%]">Confirm deletion of ALL files?</p>
      </div>
      <div id="delete-all-button-container" className='h-[50%] text-[1.25rem] w-[60%] flex flex-row justify-center items-start justify-around'>
        <button id="confirmation-deletent-button" className='confirmation-button h-[48%] w-[30%] border-[1px]  flex items-center justify-center hover:cursor-pointer bg-[#ff000069] rounded hover:bg-[#ff00007d] border-red-600' onClick={handleYes}>Yes</button>
        <button id="confirmation-delete-button" className='confirmation-button h-[48%] w-[30%] border-[1px]  flex items-center justify-center border-blue-600 rounded bg-[#002e93] hover:bg-[#0047e1]' onClick={handleNo}>No</button>
      </div>
    </div>
  )
}
